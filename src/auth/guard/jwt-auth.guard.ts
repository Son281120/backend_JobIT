...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  handleRequest(err, user, info, context: ExecutionContext) {
    ...
    if (err || !user) {
      throw (
        ...
        new UnauthorizedException(
          'Token không hợp lệ hoặc không có token ở Header token!',
        )
      );
    }
    ...
    let isExsitPermissions = permissions.find(
      (permissions) =>
        targetMethod === permissions.method &&
        targetEndpoint === permissions.apiPath,
    );

    if (targetEndpoint.startsWith('/api/v1/auth')) isExsitPermissions = true;
    if (!isExsitPermissions && !isSkipPermission) {
